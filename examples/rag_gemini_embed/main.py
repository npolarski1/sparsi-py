...
from sparsi.library.embedding_ops import GeminiEmbeddingClient
from google import genai
import sparsi.library # Registers standard ops

logger = structlog.get_logger(__name__)

# --- Gemini Vector Retriever ---

class GeminiVectorRetriever:
    def __init__(self, docs: List[Document], model: str, api_key: str):
        self.docs = docs
        self.model = model
        self.client = genai.Client(api_key=api_key)
        self.embedding_client = GeminiEmbeddingClient(self.client, model)
    ...
    async def index(self):
        ...
        self.vectors = await self.embedding_client.embed(None, texts)
    ...
    async def retrieve(self, ctx: Any, query: str, k: int) -> List[Document]:
        ...
        q_vecs = await self.embedding_client.embed(ctx, [query])
        if not q_vecs:
            return []
        q_vec = q_vecs[0]
        ...
        for i, d in enumerate(self.docs):
            score = self._cosine_similarity(q_vec, self.vectors[i])
...
# --- Custom Ops ---
# Re-using the same ops as in rag_bm25.py, but defined here for self-containment
...
async def run_workflow(question: str, kb_dir: str, verbose: bool):
    ...
    retriever = GeminiVectorRetriever(docs, "models/gemini-embedding-001", api_key)
...
async def main():
    parser = argparse.ArgumentParser(description="RAG with Gemini Embeddings example.")
    parser.add_argument("--question", default="how do I return an item?", help="The question to ask.")
    ...
    print(f"Running RAG-Gemini-Embed for: {args.question}")
    try:
        ...
        for p in result["retrieved"]:
            print(f"  [{p['source']}] cosine={p['score']:.3f}")
